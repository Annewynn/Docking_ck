...
#ce script lance, pour chaque fichier pdbqt dans base_de_donnees/docking/*/ : 
...
rm -r */
#dossier courant : Cytokinines/base_de_donnees/analyse_docking/
...
for fichier in $dir; do   #pour chaque fichier pdbqt dans docking
    #on crée un dossier portant le nom du ligand similaire(jusqu à 15 similaires et 5 récepteurs possibles par ligand donc 15*5*3=225 fichiers potentiels)
    dossier=`echo $fichier | cut -d '/' -f 3`
    if [ ! -d "$dossier" ]; then
        mkdir $dossier
    fi
    #split_states : nom du ligand, chemin du fichier pdbqt du ligand, chemin de sortie
    nom_ligand=`echo $fichier | cut -d '/' -f 4 | cut -d . -f 1`
...
#arg 3 : fichier sdf avec les similaires
    nom_ligand=`echo $fichier | cut -d '/' -f 2 | cut -d . -f 1 | cut -d '_' -f 4-10`
        dossier=`echo $fichier | cut -d '/' -f 1`
    log="../docking/$dossier/$nom_ligand.log"
    sdf="../pubchem_similaire_$dossier.sdf"
